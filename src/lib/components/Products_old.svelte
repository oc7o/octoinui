...
<script lang="ts">
	...
	const products = new ProductsQueryStore();
	...
	onMount(async () => {
		...
		// {
		// 	variables: { user: props?.user ? props.user : null, offset, limit, search }
		// }
	});
	...
	// 	search = value;
	// 	if ($products?.data != null && $products?.data.products) {
	// 		offset = 0;
...
<section class="gallery" on:mousemove={showImgContent}>
	<div class="container">
		{#if $products?.data != null && $products?.data.products && $products.data?.products?.items.length}
			<div class="grid">
				...
				<div class="btn-group mt-4">
					<button
						...
						on:click={() => {
							if (offset > 0) {
								offset -= limit;
								products.loadNextPage({ offset: offset, limit: limit });
							}
						}}>«</button
					>
		...
		{:else if $products?.error}
			<p class="text-2xl text-center">Error: {$products.error.message}</p>
		{:else}
