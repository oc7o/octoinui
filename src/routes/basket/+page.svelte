<script lang="ts">
	...
	let basket: any;
	$: basket = data.basket?.basketObjects;
	...
	async function removeBasket(webId: string) {
		removeBasketMutation.mutate({ webId });
		const r = await fetch('/api/basket/create');
		const c = await r.json();
		const cartWebId = c.createBasket.webId;
	...
	{#if basket != undefined && basket != null}
		...
		{#if basket.length != 0}
			<div class="flex justify-end my-16">
				...
				<button class="btn btn-error" on:click={() => removeBasket(data.basket?.webId)}
