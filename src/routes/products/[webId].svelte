<script context="module" lang="ts">
  import { GQL_productByWebId } from "$houdini";
  import type { LoadEvent } from "@sveltejs/kit";
  export async function load(event: LoadEvent) {
    const variables = { webId: event.params.webId };
    await GQL_productByWebId.fetch({ event, variables });
...
<script lang="ts">
  import type { productByWebId$input } from "$houdini";
  import { browser } from "$app/env";
  import { cartStore, addToCart } from "$lib/stores/cartStore";
  export let variables: productByWebId$input;

  $: browser && GQL_productByWebId.fetch({ variables });
...
{#if !$GQL_productByWebId.isFetching && $GQL_productByWebId.data?.productByWebId?.product != undefined}
  <div class="w-full carousel rounded-box h-full px-96 py-8">
    {#each $GQL_productByWebId.data?.productByWebId?.product as inventory, i}
      <div id={`#${inventory.sku}`} class="carousel-item w-full h-full card">
        <div class="card__title">
          <div class="icon">
            <a href="/"><i class="fa fa-arrow-left" /></a>
          </div>
          <h3>{$GQL_productByWebId.data?.productByWebId?.category.name}</h3>
        </div>
        <div class="card__body">
          <div class="half">
            <div class="featured_text">
              <h1>{$GQL_productByWebId.data?.productByWebId?.name}</h1>
              <p class="sub">[Sub Placeholder]</p>
              <p class="price">[Price Placeholder]</p>
            </div>
            <div class="image">
              <img
                class="rounded-xl w-full"
                src={$GQL_productByWebId.data?.productByWebId?.defaultImage
                  ?.imgUrl}
                alt=""
              />
            </div>
          ...
          <div class="half">
            <div class="description">
              <p>
                {$GQL_productByWebId.data?.productByWebId?.description}
              </p>
            </div>
  ...
  <div class="flex justify-center w-full py-2 gap-2">
    {#each $GQL_productByWebId.data?.productByWebId?.product as inventory, i}
      <a href={`#${inventory.sku}`} class="btn btn-xs">1</a>
    {/each}
  </div>
{/if}
